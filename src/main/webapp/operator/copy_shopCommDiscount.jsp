		...
		if(row.code==0){//成功
			...
			if(discountGroup!=null){
				...
				for(var d in discountGroup){
					...
					var discountType = dg.discountType;
					if(discountType == 1){//满减
						var fullCutHtml = "";//满减的html
						var tmpData = JSON.parse(dg.data);
						fullCutHtml += "<div class=\"form-group\">";
						fullCutHtml += "<label for=\"fullCut\" class=\"col-md-3 control-label\">满减（元）</label>";
						fullCutHtml += "<div class=\"col-md-8\">";
						fullCutHtml += "<div class=\"col-items col-items-2 clearfix\">";
						fullCutHtml += "<div class=\"item\">";
						fullCutHtml += "<input type=\"text\" class=\"form-control\" name=\"full\" id=\"full_"+d+"\" placeholder=\"消费金额\" value=\""+parseInt(tmpData.maxPrice)/100+"\"/>";
						fullCutHtml += "</div>";
						fullCutHtml += "<div class=\"item item-separator\">减</div>";
						fullCutHtml += "<div class=\"item\">";
						fullCutHtml += "<input type=\"text\" class=\"form-control\" name=\"minus\" id=\"minus_"+d+"\" placeholder=\"优惠金额\" value=\""+parseInt(tmpData.reducePrice)/100+"\"/>";
						fullCutHtml += "</div>";
						fullCutHtml += "</div>";
						fullCutHtml += "</div>";
						fullCutHtml += " <a href=\"javascript:void(0)\" id=\"removeDIV_"+d+"\"><img alt=\"\" src=\"${pageContext.request.contextPath}/operator/images/unfold1.png\"></a>";
						fullCutHtml += "</div>";
						$("#fullCutDIV").append(fullCutHtml);
					}else if(discountType == 3){//折扣
						var discountHtml = "";//折扣的html
						var tmpData = JSON.parse(dg.data);
						discountHtml += "<div class=\"form-group\">";
						discountHtml += "<label class=\"col-md-3 control-label\">折扣商品</label>";
						discountHtml += "<div class=\"col-md-8\">";
						discountHtml += "<div class=\"col-items col-items-2 clearfix\">";
						discountHtml += "<div class=\"item\">";
						discountHtml += "<select class=\"selectpicker\" data-live-search=\"true\" title=\"商品类型\" id=\"commType_"+d+"\">";
						<%
						if(ty!=null){
							for(int tl=0,len=ty.size(); tl<len; tl++){
						%>
							discountHtml += "<option value=\"<%=ty.get(tl).getTypeId()%>\" >";
							discountHtml += '<%=ty.get(tl).getTypeName()%>';
							discountHtml += "</option>";
						<% 
							}
						}
						...
						discountHtml += "</select>";
						discountHtml += "</div>";
						discountHtml += "<div class=\"item item-separator\"></div>";
						discountHtml += "<div class=\"item\">";
						discountHtml += "<select class=\"selectpicker\" data-live-search=\"true\" title=\"商品\" id=\"commList_"+d+"\">";
						//食品
						discountHtml += "</select>";
						discountHtml += "</div>";
						discountHtml += "</div>";
						...
						discountHtml += "</div>";
						discountHtml += "<div class=\"form-group\">";
						discountHtml += "<label for=\"discount\" class=\"col-md-3 control-label\">折扣</label>";
						discountHtml += "<div class=\"col-md-8\">";
						discountHtml += "<div class=\"input-group\">";
						discountHtml += "<input type=\"text\" class=\"form-control border-right-none\" name=\"discountVal_"+d+"\" id=\"discountVal_"+d+"\" value=\""+tmpData.percent+"\"/>";
						discountHtml += "<div class=\"input-group-addon\">%</div>";
						discountHtml += "</div>";
						discountHtml += "</div>";
						discountHtml += "<a href=\"javascript:void(0)\" id=\"removeDIV_"+d+"\"><img alt=\"\" src=\"${pageContext.request.contextPath}/operator/images/unfold1.png\"></a>";
						discountHtml += "</div>";
						$("#discountDIV").append(discountHtml);
						var typeId = 0;
						if(dg.commodityEntity==null){//商品为空时
							typeId = dg.commodityTypeEntity.typeId;
						}else{
							typeId = dg.commodityEntity.typeId;
						}
						$("select[id=commType_"+d+"]").selectpicker('val', typeId);
						$("select[id=commType_"+d+"]").selectpicker('refresh');
						var page = {};
						var param = {};
						param.typeId = typeId;
						$.ajax({
				            ...
				            success: function (data) {
				                if(data != null && data.length >0){
				                    ...
				                    $("select[id=commList_"+d+"]").append(commHtml);
				                    if(dg.commodityEntity != null){
										$("select[id=commList_"+d+"]").selectpicker('val', dg.commodityEntity.commId);
				                    }
				                    $("select[id=commList_"+d+"]").selectpicker('refresh');
				                }
				            }
				        });
					}
					$("select[id=commType_"+d+"]").on("change", function (){
						var typeId = $(this).val();
						var tmpLen = $(this).prop("id").split("_")[1];
						var page = {};
						var param = {};
					...
					$("a[id=removeDIV_"+d+"]").on("click",function (){
						var pId = $(this).parent().parent().prop("id");
						if(pId=="fullCutDIV"){//满减
							$(this).parent().remove();
						}else if(pId=="discountDIV"){//折扣
