...
$(document).ready(function(){
	...
	var purId = parent.$("#purId").val();//得到操作员的一条记录
	dealRequest(purId);
	...
	$(".btn-primary").on("click",function (){
		...
		purchase.state = 0;//状态改为审核中
		purchase.remark = "";//未审核理由改为空
	 	$.ajax({
 			...
 			if(row.code==0){//成功
 				layer.msg("修改采购单成功", {icon: 1});
 				setTimeout("closePanel()", 1000);
 				return;
 			}
 			$("#resultMessage").html("修改采购单失败");
 		})
...
function dealRequest(purId){
	$.ajax({
		...
		console.log(row);
		if(row.code==0){//成功
			$("select[id=type]").selectpicker('val', row.data.type);
			$("select[id=type]").selectpicker('refresh');//选中采购单类型
			$("select[id=shopList]").selectpicker('val', row.data.shopId);
			$("select[id=shopList]").selectpicker('refresh');//选中商店
			$("#message").val(row.data.message);//订购理由
			var purchaseDetail = row.data.purchaseDetailPages;
			if(purchaseDetail != null){
				for(var p in purchaseDetail){
					var pd = purchaseDetail[p];
					var html = "";
					html += "<div class=\"form-group\">";
					html += "<label class=\"col-md-3 control-label\">折扣商品</label>";
					html += "<div class=\"col-md-5\">";
					html += "<div class=\"col-items col-items-2 clearfix\">";
					html += "<div class=\"item\">";
					html += "<select class=\"selectpicker\" data-live-search=\"true\" title=\"商品类型\" id=\"commType_"+p+"\">";
					<%
					if(ty!=null){
						for(int tl=0,len=ty.size(); tl<len; tl++){
					%>
					...
					html += "<div class=\"item item-separator\"></div>";
					html += "<div class=\"item\">";
					html += "<select class=\"selectpicker\" data-live-search=\"true\" title=\"商品\" id=\"commList_"+p+"\">";
					//食品
					html += "</select>";
					html += "</div>";
					html += "</div>";
					html += "</div>";
					html += "<div class=\"col-md-3\">";
					html += "<input type=\"text\" class=\"form-control\" name=\"amount\" id=\"amountVal_"+p+"\" placeholder=\"请输入数量\"/>";
					html += "</div>";
					html += "<a href=\"javascript:void(0)\" id=\"removeDIV_"+p+"\"><img alt=\"\" src=\"${pageContext.request.contextPath}/operator/images/unfold1.png\"></a>";
					html += "</div>";
					$("#goodsDIV").append(html);
					
					var typeId = 0;
					if(pd.commodity == null){//商品为空时
						typeId = pd.commodity.typeId;
					}else{
						typeId = pd.commodity.typeId;
					}
					$("select[id=commType_"+p+"]").selectpicker('val', typeId);
					$("select[id=commType_"+p+"]").selectpicker('refresh');
					var page = {};
					var param = {};
					param.typeId = typeId;
					$.ajax({
			            ...
			            success: function (data) {
			                if(data != null && data.length >0){
			                    ...
			                    $("select[id=commList_"+p+"]").append(commHtml);
			                    if(pd.commodity != null){
									$("select[id=commList_"+p+"]").selectpicker('val', pd.commodity.commId);
			                    }
			                    $("select[id=commList_"+p+"]").selectpicker('refresh');
			                }
			            }
			        });
					$("select[id=commType_"+p+"]").on("change", function (){
						var typeId = $(this).val();
						var tmpLen = $(this).prop("id").split("_")[1];
						var page = {};
						var param = {};
					...
					$("input[id=amountVal_"+p+"]").val(pd.amount);//数量
					$("a[id=removeDIV_"+p+"]").on("click",function (){
						$(this).parent().remove();
					});
				}
			}
